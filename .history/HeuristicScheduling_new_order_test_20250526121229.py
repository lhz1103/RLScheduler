...
"""
5.15：
1. 运行synthetic_ex=0.3的实验

5.16:
1. 运行synthetic_ex=0.4的实验

5.17：
1. 昨天的synthetic_ex=0.4没弄好数据集和seed，重测

5.18：
1. 运行synthetic_ex=0.4的实验，并修改到达率，看看帕累托最优

5.21：
1. 运行num_nodes=8的场景试试

5.23
1. num_nodes=8时，测试constantwait

5.26
1. 测试在默认状态的性能，seed=0+episode，与2000轮训练对比（没有预测上云）：此时RL的时间略低，但费用略高
2. 试试如果加上预测上云
"""
...
class heuristic:
    def __init__(self, env, strategy = 'FIFO'):  
        ...
        self.figures_path = f"~/starburst/RLscheduler/evaluation/arr_{self.env.arrival_rate}" 
    ...
    def train(self, max_episodes=1000, update_interval=2048):
        ...
        for episode in range(max_episodes):
            ...
            state = self.env.reset(seed = 0 + episode)
            done = False
            episode_reward = 0
            episode_steps = 0
            ...
            # 记录统计数据
            jct_vec, jct_mean, jct_p95, jct_p99, wait_time_mean, wait_time_p95, wait_time_p99 = self.env.get_time_statistics()
            episode_jcts.append(jct_vec)
        ...
        np.savez_compressed(
            os.path.join(self.figures_path, f"episode_jcts_{self.schedule_policy}.npz"),
        ...
        csv_dir = os.path.expanduser("~/starburst/RLscheduler/Pareto_curves/eva_arr")
...
if __name__ == "__main__":
    ...
    arrival_rate_set = [45, 60, 90, 120, 150]  # 平均每个时隙[15, 20, 30, 40, 50]
    wait_factor_set = [0.25, 0.75, 1.25, 2.5, 3]
    privacy_rate_set = [0.05, 0.1, 0.2, 0.4, 0.8]
    job_runtime_set = [0.5, 1, 2, 4, 8]

    for arrival in arrival_rate_set:
        env = JobSchedulingEnv(arrival_rate=arrival)
