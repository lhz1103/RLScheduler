...
"""
5.15:
1.修改trace为synthetic_ex

5.17：
1. 后台运行synthetic_ex=0.4的实验；
2. 改回philly_gen，准备加上预算，测试deadline满足率，但是应该怎么测呢？
"""


class JobSchedulingEnv():
    def __init__(self, num_nodes=16, num_gpus_per_node=8, cloud_cost_weight=1, max_jobs_per_ts=20, alpha=1.0, beta=2.0, gamma=3.0, kappa=0.5, invalid_penalty=5.0, 
                 budget_factor = 0.05, wait_factor=1.25, arrival_rate = 90, privacy_rate = 0.2, job_runtime = 4, wait_policy='linearCost'):
        """
        基于PyTorch的任务调度环境
    ...
    def reset(self, seed = 2024) -> Dict[str, np.ndarray]:
        ...
        dataset_config = {  
        'dataset': 'philly_gen',
        'arrival_rate': self.arrival_rate,  # 每时隙30个任务
        'cv_factor': 1.0,
        'total_jobs': 1000,
        'seed': seed,
    ...
    def get_time_statistics(self):
        jct_array = np.array(self.jct, dtype=np.float32)
        ...
        return jct_array, jct_mean, jct_p95, jct_p99, wait_time_mean, wait_time_p95, wait_time_p99
