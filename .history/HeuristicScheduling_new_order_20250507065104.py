...
class heuristic:
    def __init__(self, env, strategy = 'FIFO_constant'):  
        
        self.env = env
        self.strategy = strategy
        self.schedule_policy, self.wait_policy = strategy.split('_')  
    ...
    def get_deadline(self, job):
        if self.wait_policy == 'constant':
            ...
            return min(48, max(1 / 12.0, 0.25 * job.runtime))  # 与运行时间呈正比，不过处于5分钟~48小时之间，以防等太久
        elif self.wait_policy == 'linearGPU':
            return job.num_gpus * 1
        elif self.wait_policy == 'linearCost':
            return job.runtime * job.num_gpus * 1
    ...
    def log_env_state(self, episode: int, timeslot: int):
        ...
        # 日志文件名为 self.strategy.txt
        log_file = os.path.join(log_dir, f"{self.strategy}.txt")
    ...
    def train(self, max_episodes=1000, update_interval=2048):
        ...
        # 必须存在这些数据文件才能使用
        # 训练结束保存数据，追加到excel中
        self.append_method_to_excel(f"~/starburst/RLscheduler/figures_new_order_budget_factor={self.env.budget_factor}/Rewards.xlsx", method_name= self.strategy, data_row = episode_rewards)

        self.append_method_to_excel(f"~/starburst/RLscheduler/figures_new_order_budget_factor={self.env.budget_factor}/Mean_JCT_hours.xlsx", method_name= self.strategy, data_row = episode_jct_mean)
        self.append_method_to_excel(f"~/starburst/RLscheduler/figures_new_order_budget_factor={self.env.budget_factor}/P95_JCT_hours.xlsx", method_name= self.strategy, data_row = episode_jct_p95)
        self.append_method_to_excel(f"~/starburst/RLscheduler/figures_new_order_budget_factor={self.env.budget_factor}/P99_JCT_hours.xlsx", method_name= self.strategy, data_row = episode_jct_p99)

        self.append_method_to_excel(f"~/starburst/RLscheduler/figures_new_order_budget_factor={self.env.budget_factor}/Mean_Wait_Time_hours.xlsx", method_name= self.strategy, data_row = episode_wait_time_mean)
        self.append_method_to_excel(f"~/starburst/RLscheduler/figures_new_order_budget_factor={self.env.budget_factor}/P95_Wait_Time_hours.xlsx", method_name= self.strategy, data_row = episode_wait_time_p95)
        self.append_method_to_excel(f"~/starburst/RLscheduler/figures_new_order_budget_factor={self.env.budget_factor}/P99_Wait_Time_hours.xlsx", method_name= self.strategy, data_row = episode_wait_time_p99)

        self.append_method_to_excel(f"~/starburst/RLscheduler/figures_new_order_budget_factor={self.env.budget_factor}/Mean_Cloud_Cost.xlsx", method_name= self.strategy, data_row = episode_cloud_cost_mean)
        self.append_method_to_excel(f"~/starburst/RLscheduler/figures_new_order_budget_factor={self.env.budget_factor}/P95_Cloud_Cost.xlsx", method_name= self.strategy, data_row = episode_cloud_cost_p95)
        self.append_method_to_excel(f"~/starburst/RLscheduler/figures_new_order_budget_factor={self.env.budget_factor}/P99_Cloud_Cost.xlsx", method_name= self.strategy, data_row = episode_cloud_cost_p99)

        self.append_method_to_excel(f"~/starburst/RLscheduler/figures_new_order_budget_factor={self.env.budget_factor}/Total_Cloud_Cost.xlsx", method_name= self.strategy, data_row = episode_total_cost)
        ratio = [total / budget if budget != 0 else 0 for total, budget in zip(episode_total_cost, episode_budget)]
        self.append_method_to_excel(f"~/starburst/RLscheduler/figures_new_order_budget_factor={self.env.budget_factor}/Cost_to_Budget_Ratio.xlsx", method_name= self.strategy, data_row = ratio)
...
if __name__ == "__main__":
    ...
    from JobSchedulingEnv_new_order import JobSchedulingEnv
    
    # 创建环境
    num_nodes = 16
    episodes = 100
    budget_factors_set = [0.001, 0.004, 0.008, 0.01, 0.04]
    wait_factor_set = [0.1, 0.5, 1, 3, 5]
    for factor in budget_factors_set:
        env = JobSchedulingEnv(num_nodes=num_nodes, budget_factor=budget_factors_set)
        
        LWTF = heuristic(env, strategy = 'LongestWaitingTimeFirst_linerCost')
        rewards_LWTF = LWTF.train(max_episodes=episodes)

        
        FIFO = heuristic(env, strategy = 'FIFO_linearCost')
        
        # 开始训练
        rewards_FIFO = FIFO.train(max_episodes=episodes)

        SJF = heuristic(env, strategy = 'SJF_linearCost')
        rewards_SJF = SJF.train(max_episodes=episodes)
        SGF = heuristic(env, strategy = 'SmallGPUsFirst_linearCost')
        rewards_SGF = SGF.train(max_episodes=episodes)

        BGF = heuristic(env, strategy = 'BigGPUsFirst_linearCost')
        rewards_BGF = BGF.train(max_episodes=episodes)

        LJF = heuristic(env, strategy = 'LongestJobFirst_linearCost')
        rewards_LJF = LJF.train(max_episodes=episodes)

        HCF = heuristic(env, strategy = 'HighestCostFirst_linearCost')
        rewards_HCF = HCF.train(max_episodes=episodes)

        LCF = heuristic(env, strategy = 'LowestCostFirst_linearCost')
