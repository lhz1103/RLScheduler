...
5.17：
1. 试试4层transformer编码器，目前是RL里效果最好的
2. 试试4层4头
"""

class ActorNetwork(nn.Module):
    def __init__(self, num_nodes: int, num_gpus: int, max_jobs: int, hidden_dim: int=256, num_heads: int = 4, embed_dim: int = 32, num_layers: int = 4):  # max_jobs就是Env里的max_jobs_per_ts
        """
        Actor网络定义
...
class CriticNetwork(nn.Module):
    def __init__(self, num_nodes: int, num_gpus: int, max_jobs: int, hidden_dim: int=256, num_heads: int = 4, embed_dim: int = 32, num_layers: int = 4):
        """
        Critic网络定义
