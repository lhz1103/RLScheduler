...
class PPO:
    def __init__(self, env, device='cuda', 
        ...
        # 存储数据路径
        self.figures_path = os.path.expanduser(f"~/starburst/RLscheduler/test_4transformer_8head_reward_mod_pretrain_100steps_64bs")
    ...
    def evaluate(self, num_episodes=10, model_path=None, deterministic=True):
        ...
        for ep in range(num_episodes):
            ...
            # 收集统计数据
            jct_mean, jct_p95, jct_p99, wait_mean, wait_p95, wait_p99 = self.env.get_time_statistics()
        ...
        df = pd.DataFrame({
            ...
            'JCT_P95': jct_p95s,
            'JCT_P99': jct_p99s,
            'Wait_Mean': wait_means,
            'Wait_P95': wait_p95s,
            'Wait_P99': wait_p99s,
            'Cost_Mean': cost_means,
            'Cost_P95': cost_p95s,
            'Cost_P99': cost_p99s,
            'Total_Cost': total_costs,
            'Budget': budgets
        })
        df.to_excel(os.path.join(eval_dir, 'evaluation_details.xlsx'), index=False)

        # 生成统计摘要
        summary_df = pd.DataFrame({
            'Metric': ['Mean', 'P95', 'P99'],
            'JCT': [np.mean(jct_means), np.mean(jct_p95s), np.mean(jct_p99s)],
            'Wait_Time': [np.mean(wait_means), np.mean(wait_p95s), np.mean(wait_p99s)],
            'Cloud_Cost': [np.mean(cost_means), np.mean(cost_p95s), np.mean(cost_p99s)],
            'Total_Cost/Budget_Ratio': [np.mean(total_costs)/np.mean(budgets), None, None]
        })
        summary_df.to_excel(os.path.join(eval_dir, 'evaluation_summary.xlsx'), index=False)
...
if __name__ == "__main__":
    ...
    ppo = PPO(env, device=device)
    # -------- 行为克隆阶段 --------
    if not os.path.exists("pretrain_actor_default_100steps_64bs.pth"):
        ppo.pretrain_imitation(steps=100, batch_size=64)

    # -------- 载入权重，再 PPO --------
    ppo.actor.load_state_dict(torch.load("pretrain_actor_default_100steps_64bs.pth", map_location=device))
    # 开始训练
    rewards = ppo.train(max_episodes=episode)
