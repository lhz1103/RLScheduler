...
class ActorNetwork(nn.Module):
    def __init__(self, num_nodes: int, num_gpus: int, max_jobs: int, hidden_dim: int=64, num_heads: int = 4):  # max_jobs就是Env里的max_jobs_per_ts
        ...
        self.cross_attn = nn.MultiheadAttention(
            embed_dim=16,
            num_heads=num_heads,
            batch_first=True,       # 直接使用 (B, L, E) 的输入输出
        )
    ...
    def forward(self, state_dict):
        ...
        # ---------- cross‑attention ----------
        # Q = job_tokens, K/V = C
        D, _ = self.cross_attn(query=job_tokens, key=C, value=C)  # (B, max_jobs, 16)

        # ---------- 纵向拼接 ----------
        tokens_cat = torch.cat([job_tokens, D], dim=1)            # (B, 2*max_jobs, 16)
        x = tokens_cat.flatten(1)                                 # (B, 2*max_jobs*16)

        # ---------- MLP & 输出 ----------
        logits = self.priority_head(self.shared_layers(x))        # (B, max_jobs)
        return logits
