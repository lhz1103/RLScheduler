...
class PPO:
    def __init__(self, env, device='cuda', 
        ...
        # 存储数据路径
        self.figures_path = os.path.expanduser(f"~/starburst/RLscheduler/figures_new_order_budget_factor={self.env.budget_factor}")
    ...
    def train(self, max_episodes=500):
        ...
        save_dir  = os.path.expanduser("~/starburst/RLscheduler/Pareto_curves/budget_factors")
        os.makedirs(save_dir, exist_ok=True)
...
if __name__ == "__main__":
    ...
    # 初始化PPO
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    env = JobSchedulingEnv(num_nodes=num_nodes, budget_factor=1) 
    ppo = PPO(env, device=device)
    # 预训练
