...
class heuristic:
    def __init__(self, env, strategy = 'FIFO_constant'):  
        ...
        self.figures_path = f"~/starburst/RLscheduler/figures_new_order_budget_factor={self.env.budget_factor}" 
    ...
    def train(self, max_episodes=1000, update_interval=2048):
        ...
        # 必须存在这些数据文件才能使用
        # 训练结束保存数据，追加到excel中
        self.append_method_to_excel(f"{self.figures_path}/Rewards.xlsx", method_name= self.strategy, data_row = episode_rewards)

        self.append_method_to_excel(f"{self.figures_path}/Mean_JCT_hours.xlsx", method_name= self.strategy, data_row = episode_jct_mean)
        self.append_method_to_excel(f"{self.figures_path}/P95_JCT_hours.xlsx", method_name= self.strategy, data_row = episode_jct_p95)
        self.append_method_to_excel(f"{self.figures_path}/P99_JCT_hours.xlsx", method_name= self.strategy, data_row = episode_jct_p99)

        self.append_method_to_excel(f"{self.figures_path}/Mean_Wait_Time_hours.xlsx", method_name= self.strategy, data_row = episode_wait_time_mean)
        self.append_method_to_excel(f"{self.figures_path}/P95_Wait_Time_hours.xlsx", method_name= self.strategy, data_row = episode_wait_time_p95)
        self.append_method_to_excel(f"{self.figures_path}/P99_Wait_Time_hours.xlsx", method_name= self.strategy, data_row = episode_wait_time_p99)

        self.append_method_to_excel(f"{self.figures_path}/Mean_Cloud_Cost.xlsx", method_name= self.strategy, data_row = episode_cloud_cost_mean)
        self.append_method_to_excel(f"{self.figures_path}/P95_Cloud_Cost.xlsx", method_name= self.strategy, data_row = episode_cloud_cost_p95)
        self.append_method_to_excel(f"{self.figures_path}/P99_Cloud_Cost.xlsx", method_name= self.strategy, data_row = episode_cloud_cost_p99)

        self.append_method_to_excel(f"{self.figures_path}/Total_Cloud_Cost.xlsx", method_name= self.strategy, data_row = episode_total_cost)
        ratio = [total / budget if budget != 0 else 0 for total, budget in zip(episode_total_cost, episode_budget)]
        self.append_method_to_excel(f"{self.figures_path}/Cost_to_Budget_Ratio.xlsx", method_name= self.strategy, data_row = ratio)

        # 写入csv文件
        df = pd.DataFrame(result_list)
        csv_dir = os.path.expanduser("~/starburst/RLscheduler/Pareto_curves/budget_factors")
...
if __name__ == "__main__":
    ...
    arrival_rate_set = [30, 45, 60, 90, 150]  # 平均每个时隙[10, 15, 20, 30, 50]
    wait_factor_set = [0.1, 0.25, 0.75, 1, 1.5]
    ...
    for factor in budget_factors_set:
        ...
        OutOfOrder = heuristic(env, strategy = 'OutOfOrder_linearCost')
        rewards_OO = OutOfOrder.train(max_episodes=episodes)
