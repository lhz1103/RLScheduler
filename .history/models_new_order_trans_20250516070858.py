...
class ActorNetwork(nn.Module):
    def __init__(self, num_nodes: int, num_gpus: int, max_jobs: int, hidden_dim: int=256, num_heads: int = 4):  # max_jobs就是Env里的max_jobs_per_ts
        """
        Actor网络定义
    ...
    def forward(self, state_dict):
        ...
        # ---------- 任务 token ----------
        job_tokens = self.job_encoder(jobs.view(-1, 3))           # (B*max_jobs, 16)
...
class CriticNetwork(nn.Module):
    def __init__(self, num_nodes: int, num_gpus: int, max_jobs: int, hidden_dim: int=256):
        """
        Critic网络定义
