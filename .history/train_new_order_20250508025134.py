...
from models_new_order import create_actor_critic_networks
...
class PPO:
    ...
    def preprocess_state(self, state):
        ...
        return {
            'jobs': torch.tensor(job_feats, dtype=torch.float32, device=self.device).unsqueeze(0),  # 添加批次维度
            'cluster': torch.tensor(state["cluster"], dtype=torch.float32, device=self.device).unsqueeze(0),
            'gpu_left': torch.tensor(state["gpu_left"], dtype=torch.float32, device=self.device).unsqueeze(0),
        }
    ...
    def plot_train_metrics(self, episode_rewards, window_size=30):
        fig_dir = os.path.expanduser(f"~/starburst/RLscheduler/figures_new_order_budget_factor={self.env.budget_factor}")
    ...
    def collect_sjf_batch(self, batch_size=1024):
        obs, acts = [], []
        env = self.env         # 用同一个环境实例
        s = env.reset(seed=self.seed_count)
        episode = 0
        while len(obs) < batch_size:
            # ----- SJF 规则 -----
            if env.visible_jobs:
                sjf_idx = np.argmin([j.runtime for j in env.visible_jobs])
                next_s, _, done, _ = env.step(sjf_idx)
                obs.append(self.preprocess_state(s))
                acts.append(sjf_idx)
                s = next_s
            else:                               # 空时隙推进
            ...
            if done:
                episode += 1
                s = env.reset(seed=self.seed_count)
        #print("total episode used in imitation:", episode)
        self.seed_count += 1
        # 堆叠成张量
        obs = {k: torch.cat([o[k] for o in obs]) for k in obs[0]}
        acts = torch.tensor(acts, device=self.device)
        return obs, acts
    
    def pretrain_imitation(self, steps=10000, batch_size=512):
        """
        用交叉熵让Actor模仿SJF
        """
        ...
        for step in range(steps):
            ...
            loss   = F.cross_entropy(logits, a_batch)  # 计算交叉熵
    ...
    def train(self, max_episodes=500):
        ...
        for episode in range(max_episodes):
            ...
            save_dir = os.path.expanduser(f"~/starburst/RLscheduler/checkpoints_new_order_budget_factor={self.env.budget_factor}/{today}")  # ← 你的目标目录
        ...
        save_dir  = os.path.expanduser("~/starburst/RLscheduler/Pareto_curves")
        os.makedirs(save_dir, exist_ok=True)
