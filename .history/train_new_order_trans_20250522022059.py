...
class PPO:
    ...
    def pretrain_imitation(self,
                       steps=3000,
                       batch_size=1024,
                       ckpt_path="pretrain_actor_default.pth"):
...
if __name__ == "__main__":
    ...
    ppo = PPO(env, device=device)
    # -------- 行为克隆阶段 --------
    if not os.path.exists("pretrain_actor_default.pth"):
        ppo.pretrain_imitation(steps=100, batch_size=64)

    # -------- 载入权重，再 PPO --------
    ppo.actor.load_state_dict(torch.load("pretrain_actor_default.pth", map_location=device))
    # 开始训练
    rewards = ppo.train(max_episodes=episode)
