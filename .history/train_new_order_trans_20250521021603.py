...
4. 试试把集群规模改小：num_nodes = 8

5.21：
1. num_nodes=8时，112回合一直没有结束，太不正常了
"""
...
class PPO:
    ...
    def collect_sjf_batch(self, batch_size=1024):  # 预训练只能用synthetic的trace，否则特别慢，不应该哇
        obs, acts = [], []
    ...
    def train(self, max_episodes=500):
        ...
        for episode in range(max_episodes):
            ...
            state = self.env.reset(seed = 2024 + 111 + episode)  # 会加载初始任务
            done = False
            episode_reward = 0
            episode_steps = 0
