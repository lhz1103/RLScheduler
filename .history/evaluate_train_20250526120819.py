...
class PPO:
    def __init__(self, env, device='cuda', 
        ...
        # 存储数据路径
        self.figures_path = os.path.expanduser(f"~/starburst/RLscheduler/evaluate")
    ...
    def evaluate(self, num_episodes=10, model_path=None, deterministic=True, scenario_tag: str = "default"):
        """
        评估训练好的策略
        Args:
            num_episodes: 评估的周期数
        ...
        # 保存评估结果
        eval_root = os.path.join(self.figures_path, "evaluation")
        eval_dir  = os.path.join(eval_root, scenario_tag)
        ...
        np.savez_compressed(
            os.path.join(eval_dir, "episode_jcts_RL.npz"),
    ...
    def train(self, max_episodes=500):
        ...
        save_dir  = os.path.expanduser("~/starburst/RLscheduler/Pareto_curves/eva_arr")
        os.makedirs(save_dir, exist_ok=True)
...
if __name__ == "__main__":
    ...
    # 初始化PPO
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    dummy_env = JobSchedulingEnv() 
    ppo = PPO(dummy_env, device=device)
    # -------- 行为克隆阶段 --------
    ...
    ppo.actor.load_state_dict(
        torch.load("~/starburst/RLscheduler/checkpoints_default/ppo_actor_2000.pth",
                   map_location=device)
    )

    for arr in arrival_rate_set:
        # 3-1) 为该场景创建新环境
        env = JobSchedulingEnv(arrival_rate=arr)
        ppo.env = env                           # 换成新环境
        ...
        # 3-3) 评估并保存
        ppo.evaluate(num_episodes=20,
                     model_path=None,          # 已经 load 了
                     deterministic=True,
                     scenario_tag=tag)
