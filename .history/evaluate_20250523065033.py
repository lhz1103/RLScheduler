...
if __name__ == "__main__":
    ...
    ppo = PPO(env, device=device)
    # -------- 行为克隆阶段 --------
    if not os.path.exists("pretrain_actor_default_3000steps_1024bs.pth"):
        ppo.pretrain_imitation(steps=3000, batch_size=1024)

    # -------- 载入权重，再 PPO --------
    ppo.actor.load_state_dict(torch.load("pretrain_actor_default_3000steps_1024bs.pth", map_location=device))
    # 开始训练
    rewards = ppo.train(max_episodes=episode)
