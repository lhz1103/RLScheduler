...
class JobSchedulingEnv():
    def __init__(self, num_nodes=8, num_gpus_per_node=8, cloud_cost_weight=1, max_jobs_per_ts=20, alpha=1.0, beta=2.0, gamma=3.0, kappa=0.5, invalid_penalty=5.0, budget_factor = 0.004, wait_factor=0.25):
        """
        基于PyTorch的任务调度环境
    ...
    def _try_local_place(self, job, gpu_free):
        ...
        """
        need = job.num_gpus
        G    = max(gpu_free)          # 每节点最大 GPU (= env.num_gpus_per_node)
        N    = len(gpu_free)
    ...
    def reset(self, seed = 2024) -> Dict[str, np.ndarray]:
        ...
        for job in init_jobs:
            if job.privacy:  # 隐私任务只能放在队列中
                ...
                job.deadline = job.num_gpus * job.runtime * self.wait_factor
                budget += job.cost
    ...
    def _get_observation(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'spent_ratio': np.array(
                [ self.total_cloud_cost / self.budget_cap ],
                dtype=np.float32
                )
        }
