...
2. 已改回seed，修复了产生0cost任务的bug，再测试修改奖励的运行：jct更低；
3. 修改费用的参数：把beta=2改为1:时间跟之前差不多，费用稍低
4. 试试把集群规模改小：num_nodes = 8，发现会出现超过集群总资源的任务，修改这个bug再运行

5.21：
1. 测试num_nodes=8的效果，并与启发式比较；
2. 测试预训练效果
"""
...
class PPO:
    def __init__(self, env, device='cuda', 
        ...
        # 存储数据路径
        self.figures_path = os.path.expanduser(f"~/starburst/RLscheduler/test_4transformer_8head_reward_mod_pretrain")
    ...
    def collect_master_batch(self, batch_size=1024):
        ...
        while len(obs_buf) < batch_size:
            if env.visible_jobs:                                  # 有任务可选
                ...
                act_buf.append(sjf_idx)                           # label

                # -------- ③ 环境前进一步 -----------------
                s, _, done, _ = env.step(sjf_idx)
            else:                                                 # 空时隙
    ...
    def pretrain_imitation(self,
                       steps=3000,
                       batch_size=1024,
                       ckpt_path="pretrain_actor.pth"):
        """纯监督训练 Actor，使其模仿专家策略。"""
        self.actor.train()
        opt = torch.optim.Adam(self.actor.parameters(), lr=3e-4)

        for step in range(1, steps + 1):
            s_batch, a_batch = self.collect_master_batch(batch_size)

            logits = self.actor(s_batch)                 # (B, max_jobs)
...
if __name__ == "__main__":
    ...
    # 初始化PPO
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    env = JobSchedulingEnv(num_nodes=8) 
    ppo = PPO(env, device=device)
    # -------- 行为克隆阶段 --------
    if not os.path.exists("pretrain_actor.pth"):
        ppo.pretrain_imitation(steps=3000, batch_size=1024)

    # -------- 载入权重，再 PPO --------
    ppo.actor.load_state_dict(torch.load("pretrain_actor.pth", map_location=device))
