...
class PPO:
    def __init__(self, env, device='cuda', 
        ...
        # 存储数据路径
        self.figures_path = os.path.expanduser(f"~/starburst/RLscheduler/test_4transformer_8head_reward_mod_8nodes_constantwait_pretrain")
...
if __name__ == "__main__":
    ...
    env = JobSchedulingEnv(num_nodes=8, wait_policy='constant') 
    ppo = PPO(env, device=device)
    # -------- 行为克隆阶段 --------
    if not os.path.exists("pretrain_actor_8node_3000steps_1024bs.pth"):
        ppo.pretrain_imitation(steps=100, batch_size=64)

    # -------- 载入权重，再 PPO --------
    ppo.actor.load_state_dict(torch.load("pretrain_actor_8node_3000steps_1024bs.pth", map_location=device))
    # 开始训练
    rewards = ppo.train(max_episodes=episode)
