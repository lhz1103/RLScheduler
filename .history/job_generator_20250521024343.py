...
"""
5.20：
修改只能产生运行时长>0且至少占用1个GPU的任务
5.21：
修改philly_gen和helios_gen占用GPU不会超过集群资源最大值
"""
...
def load_processed_jobs(dataset_config: Dict[str, Any]):  # 从配置中读取数据集类型（如 philly 或 helios等），调用对应的函数加载和处理任务数据。
    ...
    if dataset_type == 'philly':
        ...
        dataset_kwargs = {
            ...
            'max_gpus': dataset_config['max_gpus'],
        }
...
def process_philly_jobs(philly_jobs: List['JobTrace']):
    ...
    jobs = [
        j for j in jobs
        if j._run_time is not None
        and j._run_time > 0
        and j.status == 'Pass'
        and sum(len(node_dict['gpus']) for node_dict in j.attempts[-1]['detail']) > 0
    ]  # 对任务筛选，只保留了成功完成的任务，并按照提交时间排序
...
def generate_philly_gpu_jobs(philly_jobs: List['JobTrace'],
    ...
    jobs = [
        ...
        and j.status == 'Pass'
        and sum(len(node_dict['gpus']) for node_dict in j.attempts[-1]['detail']) > 0
        and sum(len(node_dict['gpus']) for node_dict in j.attempts[-1]['detail']) <= max_gpus
    ]
    jobs.sort(key=lambda j: j._submitted_time)
...
def generate_helios_jobs(helios_jobs: List['HeliosJobTrace'],
    ...
    jobs = [
        j for j in jobs
        if j._run_time is not None and (j._run_time > 0) and (j.status in ['COMPLETED', 'TIMEOUT']) and (j.num_gpus > 0) and (j.num_gpus <= max_gpus)
    ]
    jobs.sort(key=lambda j: j._submitted_time)
...
def process_helios_jobs(helios_jobs: List['HeliosJobTrace']):
    ...
    jobs = [
        j for j in jobs
        if j._run_time is not None and (j._run_time > 0) and (j.status in ['COMPLETED', 'TIMEOUT']) and (j.num_gpus > 0)
    ]
    jobs.sort(key=lambda j: j._submitted_time)
