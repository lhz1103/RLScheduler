...
class PPO:
    ...
    def update(self):
        states, actions_idx, old_log_p, rewards, dones, values = zip(*self.buffer)
        ...
        for _ in range(self.ppo_epochs):
            ...
            for start in range(0, len(idxs), self.batch_size):
                ...
                new_lp = dist.log_prob(actions_idx[mb])
    ...
    def plot_train_metrics(self, episode_rewards):
        ...
        for episode in range(max_episodes):
            ...
            while not done:
                if self.env.visible_jobs:                     # ① 有任务
                    ...
                    self.buffer.append((state, action_idx, logp, reward, done, value))
                else:                                         # ② 空时隙
                    value = self.critic(self.preprocess_state(state)).item()
                    next_state, reward, done, _ = self.env.step(-1)
                    self.buffer.append((state, -1, 0.0, reward, done, value))
