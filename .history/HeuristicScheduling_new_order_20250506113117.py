...
class heuristic:
    ...
    def train(self, max_episodes=1000, update_interval=2048):
        ...
        # 必须存在这些数据文件才能使用
        # 训练结束保存数据，追加到excel中
        self.append_method_to_excel('~/starburst/RLscheduler/figures_new_order/Rewards.xlsx', method_name= self.strategy, data_row = episode_rewards)

        self.append_method_to_excel('~/starburst/RLscheduler/figures_new_order/Mean_JCT_hours.xlsx', method_name= self.strategy, data_row = episode_jct_mean)
        self.append_method_to_excel('~/starburst/RLscheduler/figures_new_order/P95_JCT_hours.xlsx', method_name= self.strategy, data_row = episode_jct_p95)
        self.append_method_to_excel('~/starburst/RLscheduler/figures_new_order/P99_JCT_hours.xlsx', method_name= self.strategy, data_row = episode_jct_p99)

        self.append_method_to_excel('~/starburst/RLscheduler/figures_new_order/Mean_Wait_Time_hours.xlsx', method_name= self.strategy, data_row = episode_wait_time_mean)
        self.append_method_to_excel('~/starburst/RLscheduler/figures_new_order/P95_Wait_Time_hours.xlsx', method_name= self.strategy, data_row = episode_wait_time_p95)
        self.append_method_to_excel('~/starburst/RLscheduler/figures_new_order/P99_Wait_Time_hours.xlsx', method_name= self.strategy, data_row = episode_wait_time_p99)

        self.append_method_to_excel('~/starburst/RLscheduler/figures_new_order/Mean_Cloud_Cost.xlsx', method_name= self.strategy, data_row = episode_cloud_cost_mean)
        self.append_method_to_excel('~/starburst/RLscheduler/figures_new_order/P95_Cloud_Cost.xlsx', method_name= self.strategy, data_row = episode_cloud_cost_p95)
        self.append_method_to_excel('~/starburst/RLscheduler/figures_new_order/P99_Cloud_Cost.xlsx', method_name= self.strategy, data_row = episode_cloud_cost_p99)

        self.append_method_to_excel('~/starburst/RLscheduler/figures_new_order/Total_Cloud_Cost.xlsx', method_name= self.strategy, data_row = episode_total_cost)
        ratio = [total / budget if budget != 0 else 0 for total, budget in zip(episode_total_cost, episode_budget)]
        self.append_method_to_excel('~/starburst/RLscheduler/figures_new_order/Cost_to_Budget_Ratio.xlsx', method_name= self.strategy, data_row = ratio)
