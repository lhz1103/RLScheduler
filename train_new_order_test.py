...
from JobSchedulingEnv_new_order_test import JobSchedulingEnv
from models_new_order_test import create_actor_critic_networks
"""
5.13：
修改以下内容：
1. batch_size=512
2. self.buffer = deque(maxlen=8192)
3. ppo_epochs=3
4. actor_lr=1e-4, 
    critic_lr=3e-4,
5.15：
...
class PPO:
    def __init__(self, env, device='cuda', 
        ...
        # 存储数据路径
        self.figures_path = os.path.expanduser(f"~/starburst/RLscheduler/test_4head_16embed_4atten_reward_mod_more_batchs_synthetic_ex_arrival_rate={self.env.arrival_rate}")
    ...
    def train(self, max_episodes=500):
        ...
        save_dir  = os.path.expanduser("~/starburst/RLscheduler/Pareto_curves/ex_arrival_rate_oldmodel")
        os.makedirs(save_dir, exist_ok=True)
